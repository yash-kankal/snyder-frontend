...
export default function MovieDetails({ routeId } = {}) {
  ...
  useEffect(() => {
    ...
    Promise.all([getUserSavedMovieIds(user.id), getUserFavoriteIds(user.id), getWatchedIds(user.id)])
      .then(([savedIds, favIds, watchedIds]) => {
        ...
        setWatched(watchedIds.has(String(id)))
      })
      .catch(() => {})
  }, [user, id])
  ...
  const handleWatched = useCallback(async () => {
    ...
    try {
      if (next) {
        await addToWatched(user.id, movie?.id, movie?.title, movie?.poster_path, 'movie')
        showToast('Added to your diary')
      } else {
        await removeFromWatched(user.id, movie?.id)
      }
    ...
    const fetchAll = async () => {
      ...
      try {
        ...
        const data = await cachedFetch(
          `${API_BASE_URL}/movie/${id}?append_to_response=credits,videos,images,release_dates,watch/providers,recommendations,reviews,external_ids&include_image_language=en,null`,
...
<div className="detail-info-row">
  ...
  <div className="detail-info-left">
    ...
    {/* Merged card: description only (stats moved to Where to Watch card) */}
  ...
  <div className="detail-info-right">
    <div className="detail-card detail-hero-watch">
      ...
      <div className="watch-stats-list">
        {movie.release_date && <div className="watch-stat-row"><svg viewBox="0 0 20 20" fill="#AB8BFF" className="meta-icon"><path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd"/></svg><span className="detail-stat-value">{new Date(movie.release_date).toLocaleDateString('en-US', { day: 'numeric', month: 'short', year: 'numeric' })}</span></div>}
        {runtime && <div className="watch-stat-row"><svg viewBox="0 0 20 20" fill="#60a5fa" className="meta-icon"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd"/></svg><span className="detail-stat-value">{runtime}</span></div>}
        {movie.original_language && <div className="watch-stat-row"><svg viewBox="0 0 20 20" fill="#34d399" className="meta-icon"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM4.332 8.027a6.012 6.012 0 011.912-2.706C6.512 5.73 6.974 6 7.5 6A1.5 1.5 0 019 7.5V8a2 2 0 004 0 2 2 0 011.523-1.943A5.977 5.977 0 0116 10c0 .34-.028.675-.083 1H15a2 2 0 00-2 2v2.197A5.973 5.973 0 0110 16v-2a2 2 0 00-2-2 2 2 0 01-2-2 2 2 0 00-1.668-1.973z" clipRule="evenodd"/></svg><span className="detail-stat-value">{LANG_NAMES[movie.original_language] || movie.original_language.toUpperCase()}</span></div>}
        {certification && <div className="watch-stat-row"><svg viewBox="0 0 20 20" fill="#f59e0b" className="meta-icon"><path fillRule="evenodd" d="M10 1.944A11.954 11.954 0 012.166 5C2.056 5.649 2 6.319 2 7c0 5.225 3.34 9.67 8 11.317C14.66 16.67 18 12.225 18 7c0-.682-.057-1.35-.166-2.001A11.954 11.954 0 0110 1.944zM11 14a1 1 0 11-2 0 1 1 0 012 0zm0-7a1 1 0 10-2 0v3a1 1 0 102 0V7z" clipRule="evenodd"/></svg><span className="detail-stat-value">{certification}</span></div>}
        {movie.budget > 0 && <div className="watch-stat-row"><svg viewBox="0 0 20 20" fill="#fbbf24" className="meta-icon"><path d="M8.433 7.418c.155-.103.346-.196.567-.267v1.698a2.305 2.305 0 01-.567-.267C8.07 8.34 8 8.114 8 8c0-.114.07-.34.433-.582zM11 12.849v-1.698c.22.071.412.164.567.267.364.243.433.468.433.582 0 .114-.07.34-.433.582a2.305 2.305 0 01-.567.267z"/><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-13a1 1 0 10-2 0v.092a4.535 4.535 0 00-1.676.662C6.602 6.234 6 7.009 6 8c0 .99.602 1.765 1.324 2.246.48.32 1.054.545 1.676.662v1.941c-.391-.127-.68-.317-.843-.504a1 1 0 10-1.51 1.31c.562.649 1.413 1.076 2.353 1.253V15a1 1 0 102 0v-.092a4.535 4.535 0 001.676-.662C13.398 13.766 14 12.991 14 12c0-.99-.602-1.765-1.324-2.246A4.535 4.535 0 0011 9.092V7.151c.391.127.68.317.843.504a1 1 0 101.511-1.31c-.563-.649-1.413-1.076-2.354-1.253V5z" clipRule="evenodd"/></svg><span className="detail-stat-value">Budget · ${movie.budget >= 1e9 ? (movie.budget / 1e9).toFixed(1) + 'B' : Math.round(movie.budget / 1e6) + 'M'}</span></div>}
        {movie.revenue > 0 && <div className="watch-stat-row"><svg viewBox="0 0 20 20" fill="#4ade80" className="meta-icon"><path fillRule="evenodd" d="M3 3a1 1 0 000 2v8a2 2 0 002 2h2.586l-1.293 1.293a1 1 0 101.414 1.414L10 15.414l2.293 2.293a1 1 0 001.414-1.414L12.414 15H15a2 2 0 002-2V5a1 1 0 100-2H3zm11 4a1 1 0 10-2 0v4a1 1 0 102 0V7zm-3 1a1 1 0 10-2 0v3a1 1 0 102 0V8zM8 9a1 1 0 00-2 0v2a1 1 0 102 0V9z" clipRule="evenodd"/></svg><span className="detail-stat-value">Grossed · ${movie.revenue >= 1e9 ? (movie.revenue / 1e9).toFixed(2) + 'B' : Math.round(movie.revenue / 1e6) + 'M'}</span></div>}
        {movie.status && <div className="watch-stat-row"><svg viewBox="0 0 20 20" fill="#4ade80" className="meta-icon"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/></svg><span className={`detail-stat-value stat-status stat-status--${movie.status.toLowerCase().replace(/\s+/g, '-')}`}>{movie.status}</span></div>}
      </div>
